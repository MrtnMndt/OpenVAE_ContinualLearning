...
class CIFAR_ResNet(nn.Module):
    def __init__(self, device, num_classes, num_colors, args):
        ...
        self.decoder = nn.Sequential(OrderedDict([
            ...
            ('decoder_block4', CIFAR_ResBasickBlock(in_planes=self.nChannels[0], planes=self.out_channels, is_transposed=True)),
            # ('decoder_act1', nn.Tanh()),
            ]))

        from .gan import Discriminator
        self.discriminator = Discriminator(self.out_channels, self.num_classes ,self.nChannels, args)
...
class CIFAR_ResNet_proj(nn.Module):
    def __init__(self, device, num_classes, num_colors, args):
        super(CIFAR_ResNet_proj, self).__init__()
        ...
        self.decoder = nn.Sequential(OrderedDict([
            # ('decoder_uplatent', nn.Linear(self.enc_spatial_dim_x * self.enc_spatial_dim_y *
            #                             self.enc_channels, self.enc_channels*4*4*8)), # 64,4,4,8
            ('decoder_block1', CIFAR_ResNetworkBlock(self.enc_channels*4, self.enc_channels*4, res_block=0, stack=1, is_transposed=True)), # 32,8,8,4
...
class CIFAR_ResNet_conv_proj(nn.Module):
    def __init__(self, device, num_classes, num_colors, args):
        ...
        super(CIFAR_ResNet_conv_proj, self).__init__()
        ...
        self.encoder = nn.Sequential(OrderedDict([
            ...
            ('encoder_block3', self._make_layer(CIFAR_ResNetworkBlock, stack=1)),
            ('encoder_block4', self._make_layer(CIFAR_ResNetworkBlock, stack=2)),
            # ('encoder_avgpool1', nn.AvgPool2d(8)),  # b,64,8,8
            ]))
        ...
        self.latent_mu = nn.Sequential(
                        nn.Conv2d(self.enc_channels, self.latent_dim, kernel_size = 3, stride=2, padding = 1), # 60,4,4
                        nn.Flatten()
                        )
        ...
        self.classifier = nn.Sequential(nn.Linear(int(self.latent_dim * self.enc_spatial_dim_x *self.enc_spatial_dim_y/4) , num_classes, bias=False))
        # self.classifier = nn.Conv2d(self.latent_dim, num_classes, kernel_size = (int(self.enc_spatial_dim_x/2),int(self.enc_spatial_dim_y/2)), padding =0)

        self.latent_decoder = nn.Sequential(OrderedDict([
                                    ('latent_conv1',nn.ConvTranspose2d(self.latent_dim, self.enc_channels*4, kernel_size=1, stride=1)),
                                    ]))
        self.decoder = nn.Sequential(OrderedDict([
            # ('decoder_uplatent', nn.Linear(self.enc_spatial_dim_x * self.enc_spatial_dim_y *
            #                             self.enc_channels, self.enc_channels*4*4*8)), # 64,4,4,8
    ...
    def decode(self, z):
        z = z.view(z.shape[0], self.latent_dim, int(self.enc_spatial_dim_y/2), int(self.enc_spatial_dim_x/2))
        z = self.latent_decoder(z)
        x = self.decoder(z)
        return x

    def generate(self):
        z = torch.randn(self.batch_size, self.latent_dim*16).to(self.device)
        x = self.decode(z)
        # x = torch.Tanh(x)
        # x = torch.sigmoid(x)
        return x
