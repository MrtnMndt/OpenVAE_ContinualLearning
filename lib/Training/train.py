...
#         #update Discriminator == Encoder in ed case
#         class_samples, recon_samples, mu, std = model(inp)
#         real_z = model.module.reparameterize(mu,std)
#         real_z = model.module.forward_D(real_z)
#         n,b,c,x,y = recon_samples.shape
#         fake_class_samples, fake_mu, fake_std = model.module.forward_E(recon_samples.view(n*b,c,x,y))
#         fake_z = model.module.reparameterize(fake_mu, fake_std)
#         fake_z = model.module.forward_D(fake_z)
        
#         # Needed to add GAN_criterion on KL
#         GAN_criterion = torch.nn.BCELoss()#reduction='sum')
#         GAN_D_loss = GAN_criterion(real_z, torch.ones_like(real_z).float()) + GAN_criterion(fake_z, torch.zeros_like(fake_z).float())
#         GAN_G_loss = GAN_criterion(fake_z, torch.ones_like(fake_z).float()) + GAN_criterion(real_z, torch.zeros_like(real_z).float())
#         D_losses.update(GAN_D_loss.item(), inp.size(0))
#         G_losses.update(GAN_G_loss.item(), inp.size(0))
#         GAN_D_loss, GAN_G_loss = torch.mean(GAN_D_loss,dim=0), torch.mean(GAN_G_loss,dim=0)

#         # add the individual loss components together and weight the KL term.
#         # loss = GAN_D_loss + GAN_G_loss
